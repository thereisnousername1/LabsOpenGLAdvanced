...
// layout (binding = 0) uniform sampler2D Tex1; // multiple texture is not use
// layout (binding = 1) uniform sampler2D Tex2;

uniform sampler2D NoiseTex; // lab 9.3

uniform float LowThresHold; // lab 9.3
uniform float HighThresHold; // lab 9.3

uniform vec3 PaintColor = vec3(1.0);    // lab 9.4
...
//// specular lighting is meant to represent glare upon an object's surface
    ...
    */
    ...
    //                      base texture
    // vec3 texColor = texture(Tex1, TexCoord).rgb; // lab 9.3
...
void main()
{
    ...
    /* former alpha clipping logic, for referenced only
    // lab 4.3
    ...
    */
    ...
    // FragColor = vec4(phongModel(position.xyz, normal), 1.0);
